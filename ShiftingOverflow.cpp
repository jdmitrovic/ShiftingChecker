...
void ShiftingChecker::checkPreStmt(const BinaryOperator *B,
  ...
  QualType typeOfRHS = DV.getType();

  if(typeOfRHS->isSignedIntegerType() && C.isNegative(DV)){
...
bool ento::shouldRegisterShiftingChecker(const LangOptions &LO) {
  return true;
}
...
registry.addChecker <ShiftingChecker>("alpha.core.ShiftingChecker",
                                              "Checks shifting overflow and shifting"
                                              "by negative values");
}